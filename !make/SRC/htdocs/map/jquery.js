 ...
 */ (function ($) {
        ...
        Tooltip: function (settings) {
            ...
            return this.each(function () {
                ...
                $(this).hover(save, function () {
                    ...
                    if (helper.parent.css("overflow-y") == "auto") $.Tooltip.timeout = setTimeout(hide, 300);
                    else hide();
                });
            });
    ...
    function handle(event) {
        ...
        if (this.tSettings.delay) tID = setTimeout(show, this.tSettings.delay);
        else show();
    ...
    function save(event, x, y) {
        ...
        if ($.Tooltip.blocked || this == current || !this.tooltipText) return;
        if ($.Tooltip.timeout) clearTimeout($.Tooltip.timeout);
        ...
        if (this.tSettings.bodyHandler) {
            ...
            if (!html.length) $.Tooltip.skipped = true;
            if (typeof html == "object") helper.body.html(html.html);
            else helper.body.html(html);
            ...
            if (typeof this.tSettings.oddColor != "undefined") jQuery("tr:odd", helper.body).css("background-color", this.tSettings.oddColor);
        } else {
            helper.title.html(title).show();
            helper.body.hide();
        }
        // if element has href or src, add and show it, otherwise hide it
        if (this.tSettings.showURL && $(this).url()) helper.url.html($(this).url().replace('http://', '')).show();
            ...
            if (!track_children) {
                ...
                if (top + height > wheight - 10) {
                    ...
                    if (delta_bottom > delta_top) {
                        ...
                        if (delta_top < 10) helper.parent.css("overflow-y", "auto");
                    }
                }
                if (scrollable) helper.parent.css("overflow-y", "auto");

                // set left position
                if (left + width + $.Tooltip.defaults.left > wwidth - 10) {
					//alert(wwidth - width - $.Tooltip.defaults.left);
    ...
    function update(event) {
        function tracking_enter() {
            if ($.Tooltip.timeout_tracking) clearTimeout($.Tooltip.timeout_tracking);
        ...
        if (event && track_children) {
            ...
            while (tip_parent) {
                if (tip_parent && tip_parent.tSettings) break;
                tip_parent = jQuery(tip_parent).parent();
                if (!jQuery(tip_parent).size() || !tip_parent) tip_parent = null;
                else tip_parent = jQuery(tip_parent).get(0);
            }
            ...
            if (last_target != curr_target) {
                ...
                if (scrollable) helper.parent.css("overflow-y", "auto");
                // set left position
                if (left + width + $.Tooltip.defaults.left > wwidth - 10) helper.parent.css("left", wwidth - width - $.Tooltip.defaults.left);
                // bind scrollable event
                if (helper.parent.css("overflow-y") == "auto") {
                    helper.parent.css("width", (width + 10) + "px");
                ...
                if (need_hide) return;
            } else if (helper.parent.css("overflow-y") == "auto") return;
        }
        if (!helper.body.html().length || helper.parent.css("overflow-y") == "auto" || !track_children) return;
        if (event) {
    ...
    function hide(event) {
        if ($.Tooltip.timeout_tracking) return;
        $.Tooltip.timeout = null;
